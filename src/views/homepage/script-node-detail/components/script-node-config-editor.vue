<script setup lang="ts">
import {RefreshRight, FullScreen} from "@element-plus/icons-vue";
import {RichTextType} from "@/types/richTextType.ts";
import RichText from "@/components/rich-text/rich-text.vue";
import {onMounted, ref} from "vue";
import VortexaIcon from "@/components/vortexa-icon.vue";
import {ElMessage} from "element-plus";
import {queryScriptNodeConfigNetwork, updateScriptNodeConfigNetwork} from "@/api/script-node.ts";
import ContentBlock from "@/components/content-block/content-block.vue";

const props = defineProps<{
  scriptNodeName: string
}>()

const scriptNodeConfigRichText = ref()
const launchConfigRichText = ref()

const loading = ref(false)
// 刷新
const reloadData = () => {
  queryScriptNodeConfig()
}

// 保存
const saveLaunchConfig = () => {
  const newConfig = launchConfigRichText.value.textContent
  if (scriptNodeConfigRichText.value === newConfig) {
    ElMessage.info({
      message: 'config did not edited'
    })
  } else {
    loading.value = true
    updateScriptNodeConfigNetwork(
      props.scriptNodeName,
      newConfig
    ).then(result=>{
      if (result.success) {
        ElMessage.success({
          message: 'scriptNode config update success'
        })
      }
    }).finally(() => loading.value = false)
  }
}

const queryScriptNodeConfig = () => {
  loading.value = true
  queryScriptNodeConfigNetwork(props.scriptNodeName).then(result => {
    if (result.success) {
      scriptNodeConfigRichText.value = result.data
    } else {
      ElMessage.error({
        message: 'query script node config fail'
      })
    }
  }).finally(() => loading.value = false)
}

const isFullScreen = ref(false)

onMounted(() => {
  queryScriptNodeConfig()
})
</script>

<template>
  <content-block>
    <template #header>
      <div class="clearfix" style="margin-bottom: 6px;">
        <el-link style="line-height: 32px;vertical-align: bottom;"
                 type="danger"
                 truncated>
          Tips: please read guideline before edit it
        </el-link>
        <el-button-group style="float: right">
          <el-button @click="saveLaunchConfig">
            <vortexa-icon name="save" size="14"/>
          </el-button>
          <el-button @click="reloadData">
            <el-icon>
              <RefreshRight/>
            </el-icon>
          </el-button>
          <el-button @click="isFullScreen = true">
            <el-icon>
              <FullScreen/>
            </el-icon>
          </el-button>
        </el-button-group>
      </div>
    </template>
    <rich-text
      ref="launchConfigRichText"
      class="rich-text-wrapper"
      :class="{ 'rich-text-fullscreen': isFullScreen }"
      :textType="RichTextType.yaml"
      :content="scriptNodeConfigRichText"
      editable
    />

    <el-button v-if="isFullScreen"
               type="text"
               class="fullscreen-button"
               @click="isFullScreen = false">
      <vortexa-icon name="fullscreen-exit" size="24"/>
    </el-button>
  </content-block>
</template>

<style scoped>
.rich-text-wrapper {
  height: 400px;
  border: 1px solid #ccc;
  overflow-y: scroll;
}

.rich-text-fullscreen {
  position: fixed !important;
  top: 0;
  left: 0;
  right: 0;
  bottom: 0;
  height: 100vh !important;
  width: 100vw !important;
  z-index: 9999;
  background: white;
}

.fullscreen-button {
  position: fixed !important;
  top: 10px;
  right: 10px;
  height: 30px;
  width: 30px;
  z-index: 9999;
  color: #4a4a4a;
}

</style>
